          ...
          <div class="modal-body">
            <form id="AddForm">
              <div>
                ...
                <div class="input-text" :class="{ 'input-error': validationErrors.cons_type }">
                  <select v-model="cons_type" class="input-style" required>
                    <option value=""></option>
                ...
                <div class="input-text" :class="{ 'input-error': validationErrors.move }">
                  <select v-model="move" class="input-style" required>
                    <variants  name="moving_journal" :selected="move" />
                  </select>
  ...
  export default {
    ...
    methods: {
      validateForm() {
        ...
        const requiredFields = ['lot', 'cons_type', 'manufacturer', 'analyzer', 'financing', 'expiration_date', 'balance', 'sink', 'move']
      ...
      async onSave() {
        ...
        try {
          const lotRes = await this.$http.post('http://localhost:8000/api/analyzersconsumables_lot/', {
            lot: this.lot,
          ...
          await this.$http.post('http://localhost:8000/api/analyzersconsumables_move/', {
            box: createdLotId,
            expenditure: null,
            sink: this.sink,
            move: this.move,
          ...
          if (createdLotId) {
            try {
              await this.$http.delete(`http://localhost:8000/api/analyzersconsumables_lot/${createdLotId}/`,{headers: {authorization: `Bearer ${localStorage.access_token}`}})
