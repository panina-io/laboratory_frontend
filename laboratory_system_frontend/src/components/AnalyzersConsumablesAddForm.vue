    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Добавление расходника для анализатора</h5>
          ...
          <form id="AddForm">
            <div>
              ...
              <div class="input-text" :class="{ 'input-error': validationErrors.cons_type }">
                <label class="edit_component">Тип</label>
                <select v-model="cons_type" class="input-style" required>
                  <option value=""></option>
                  <variants name="consumables_type" :selected="cons_typer" />
                </select>
              </div>
...
export default {
  ...
  methods: {
    validateForm() {
      ...
      const requiredFields = ['lot', 'cons_type', 'manufacturer', 'analyzer', 'financing', 'expiration_date', 'balance', 'sink']
